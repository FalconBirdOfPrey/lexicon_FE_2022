  ...
  return (
    <div className='listItem'>
      { change
        ...
        : <p onClick={() => toggleComplete(item)} className={`product-title ${item.completed && 'completed'}`}>{ item.product }</p>
      }
    
        <div className="buttons">
            <button onClick={toggleChange} className="btn btn-change"><i className="fa-solid fa-pen"></i></button>
            <button onClick={() => removeItem(item.id)} className="btn btn-delete"><i className="fa-solid fa-recycle"></i></button>
        </div>
    </div>
  )
}
